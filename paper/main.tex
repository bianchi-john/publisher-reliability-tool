...
entirely, and the guard covers single articles and publisher
readings alike. PRT is equally careful about what this does \emph{not} prove:
...
authors are discarded. A publisher class is never stored: it is derived per
model over that model's leakage-safe articles under three versioned rules --
\texttt{majority\_vote} (used in the study), \texttt{ordinal\_mean}
(class $=\lfloor\mu+0.5\rfloor$) and \texttt{mean\_probabilities} -- and
recomputed whenever the rule or the counted articles change.
...
RoBERTa-large). Opening a \emph{publisher} shows one class per model, read from
that model's leakage-safe runs; switching the rule or unticking articles
recomputes it live, exposing the verdict's sensitivity to both choices. We close
on the CSV ledgers, beside the browser, showing the new run.
