...
from those runs under three documented formulas. Two properties
...
offline-capable workspace over a prediction-only release of 34,564 article runs
...
outlives a deleted data directory without ever entering version control; the
shipped release is never rewritten by the running application, and one confirmed
action purges every local run, saved body and the mirror itself.
...
CPU reference fixture. The study also fine-tuned Llama~3~8B~\cite{llama2024herd}
and Mistral~24B~\cite{jiang2024mixtral}; we did not ship them, since each fold
needs CUDA and several gigabytes while the encoders reach comparable accuracy.
The design anticipates one: identity is content-addressed over output-relevant
settings, not an architecture name, and the leakage guard keys folds by article.
